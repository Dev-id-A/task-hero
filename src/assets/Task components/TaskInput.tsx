...
function TaskInput({lang, setTaskInput, setTimesInput, setDifficult}:taskInterface) {
  ...
  return (
    <div className="size-full flex flex-col -center justify-center gap-8 px-5 text-center text-xl">
      ...
      children={<select className="w-full text-center" name="difficult" id="select-difficult" onChange={(e)=> setDifficult?.(e.target.value)}>
          {difficults.map((mode)=>{
