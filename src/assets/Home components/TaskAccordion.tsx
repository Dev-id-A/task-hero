            ...
            <AccordionContent>
              ...
              className="md:grid md:grid-cols-2 lg:grid-cols-3">
                {taskState.map((object, i)=>{
                  ...
                  key={i + "div"} className={`min-h-100 flex flex-col items-center justify-center border-3 border-black m-8
                  ${nightMode ? "bg-[#262626]":"bg-blue-200"} ${object.completed && "hidden"}`}>
                      <TaskToDo key={i} 
                      {...{nightMode, lang, object, reduceTimes, eraseTask, setEraseWindow,
                ...
                className={`min-h-100 flex flex-col items-center justify-center border-3 border-black m-8
                ${nightMode ? "bg-[#262626]":"bg-blue-200"}`}>
                  {addTask ?
                  (<TaskInput {...{nightMode, lang, taskState, setTaskState, setAddTask, recurrent, alertWindowsStates}}/>):
                  (<AddTaskBtn {...{nightMode, addTask, setAddTask}}/>)}
                </motion.div>
              </section>
            </AccordionContent>
        </AccordionItem>
