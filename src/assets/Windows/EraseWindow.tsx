...
function EraseWindow({lang, windowBool, windowBoolSetter, eraserFnc, alertText}: {lang:Lang} & eraseWindowInterface) {
    ...
    ${!windowBool && "hidden"}`}>
      <div className="h-50 p-10 bg-blue-200 border-1 border-black flex flex-col items-center justify-center gap-5 font-bold">
        ...
        <div className="flex flex-col w-1/2 gap-1">
            <button className="border-1 border-black bg-green-500 w-full" onClick={()=> {eraserFnc?.(true); windowBoolSetter?.(false)}}>{alertJson.yes[lang]}</button>
            <button className="border-1 border-black bg-red-500 w-full" onClick={()=> windowBoolSetter?.(false)}>{alertJson.no[lang]}</button>
        </div>
